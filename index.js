...
async function getMainPage( browser ) {
    ...
    let last_version = await page.evaluate( ( ) => {
        let textContent = document.querySelector( 'div[itemprop=articleBody]' )
            .querySelector( 'p' )
            .nextElementSibling
            .textContent;
...
async function parseSubPage( link, browser ) {
    ...
    let sp_sections = await page.evaluate( ( objLinks ) => {
        let subpage_sections = Array.from( document.querySelectorAll( 'div[itemtype="http://developers.google.com/ReferenceObject"]' ) )
            .map( ( referenceObject ) => {
                ...
                return {
                    title: h2.id,
                    path: h2.nextElementSibling.querySelector( 'span[itemprop="path"]' ).innerText,
                    name: h2.nextElementSibling.querySelector( 'span[itemprop="name"]' ).innerText,
    ...
    sp_sections.forEach( ( referenceObject ) => {
        ...
        docLinks[ `${referenceObject.path}.${referenceObject.name}` ] = `http://huasofoundries.github.io/google-maps-documentation/${referenceObject.title}.html`; // eslint-disable-line max-len
...
async function startParsing( ) {
    ...
    let readmeBuffer = await fs.readFileAsync( path.resolve( `${__dirname}/README.md` ) ),
        readmeString = readmeBuffer.toString( 'UTF8' ).split( '### Last Update' )[ 0 ].trim( );
...
`;
  ...
  - [${child.title}](docs/${child.title}.md)`;
    ...
    await fs.writeFileAsync( path.resolve( `${__dirname}/docs/README.md` ), docsReadme );
    await fs.writeFileAsync( path.resolve( `${__dirname}/docs/index.md` ), docsReadme );


    await fs.writeFileAsync( 'links.json', JSON.stringify( linksObj, null, 4 ), 'utf-8' );

    await Promise.all( promises );
    await fs.writeFileAsync( 'docLinks.json', JSON.stringify( docLinks, null, 4 ), 'utf-8' );
