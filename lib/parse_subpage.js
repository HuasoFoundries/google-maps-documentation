...
async function parseSubPage({link, browser, linksObj = {}, docLinks = {}}) {
  ...
  let sp_sections = ((theArg) => {
    ...
    let subpage_sections = Array.from(
      ...
      try {
        let innerText = referenceObject.querySelector('h2').textContent;
        debug({innerText});
        referenceObject.querySelector('h2').innerHTML = innerText;
        ...
        var heading = referenceObject.querySelector('h2'),
          h2 = heading,
          pathElement = referenceObject.querySelector('span[itemprop="path"]'),
          nameElement = referenceObject.querySelector('span[itemprop="name"]'),
          result = {
            title: h2.id,
        ...
        try {
          ...
          result.content = referenceObject.innerHTML;
          return result;
        } catch (errRef) {
          console.log({errRef});
          return result;
